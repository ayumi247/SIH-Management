...
def create_admin_request(db: Session, admin_in: AdminCreate) -> User:
    ...
    college = db.exec(select(College).where(College.name == admin_in.college_name)).first()
    if not college:
    ...
    if existing_admin:
        raise BadRequestException(detail="College already has an Admin or pending Admin request")

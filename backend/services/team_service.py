...
def create_team(db: Session, team_in: TeamCreate, user: User) -> Team:
    ...
    team = db.exec(select(Team).where(Team.name == team_in.name, Team.college_id == user.college_id)).first()
    if team:
...
def add_user_to_team(db: Session, team_id: uuid.UUID, target_user: User) -> None:
    ...
    if target_user.college_id != team.college_id:
        raise BadRequestException(detail="User must belong to the same college as the team")
    ...
    if len(team.members) == 5:
        ...
        if not has_female and target_user.gender.lower() != "female":
            raise BadRequestException(detail="You need atleast one female member in the team")
            
    target_user.team_id = team.id
    db.add(target_user)
    db.commit()

def admin_update_team_status(db: Session, team_id: uuid.UUID, new_status: TeamStatus, admin: User) -> Team:
    team = db.get(Team, team_id)
    ...
    if new_status == TeamStatus.Shortlisted and team.status != TeamStatus.Shortlisted:
        count = len(db.exec(select(Team).where(Team.college_id == admin.college_id, Team.status == TeamStatus.Shortlisted)).all())
        if count >= 45:
        ...
        count = len(db.exec(select(Team).where(Team.college_id == admin.college_id, Team.status == TeamStatus.Waitlisted)).all())
        if count >= 5:

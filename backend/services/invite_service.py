    ...
    if invite_in.target_user_id:
        ...
        if not target_user or target_user.college_id != sender.college_id:
            raise NotFoundException(
                detail="Target user not found or in different college"
            )
        ...
        if not target_team or target_team.college_id != sender.college_id:
            raise NotFoundException(
                detail="Target team not found or in different college"
            )
        if sender.team_id:
        ...
        req = JoinRequest(sender_id=sender.id, target_team_id=target_team.id)
...
def update_invite_status(
    db: Session, invite_id: uuid.UUID, new_status: InviteStatus, current_user: User
):
